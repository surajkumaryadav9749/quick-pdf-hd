  ...
  <section aria-labelledby="contact-info-heading" className="bg-white py-16 sm:py-20 lg:py-24">
    <Container>
      <div className="mx-auto mb-14 max-w-3xl text-center">
        <p className="inline-flex rounded-full bg-blue-100 px-4 py-2 text-sm font-semibold text-blue-700">Contact information</p>
      ...
      <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
        {contactInfo.map((item) => (
          <article key={item.title} className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm transition-all duration-300 hover:-translate-y-2 hover:border-blue-200 hover:shadow-xl">
            <div aria-hidden="true" className="mb-6 inline-flex rounded-2xl bg-blue-100 p-4 text-blue-600">{item.icon}</div>
            <h3 className="text-2xl font-bold text-slate-900">{item.title}</h3>
            <p className="mt-3 break-words text-lg font-semibold text-blue-600">{item.value}</p>
